...
_.last = function(array, n) {
  ...
  if(n < 0) return array.slice(n, array.length -1);
...
_.filter = function(collection, test) {
    ...
    _.each(collection, function(value, pos, collection) {
        if(test(value, pos, collection)) trues.push(value);
    });
    return trues;
};
...
*/
_.reject = function(collection, funct) {
    const rejected = [];
    _.filter(collection, function(value, pos, collection) {
        if(!funct(value, pos, collection)) rejected.push(value);
    });
    return rejected;
};
...
_.partition = function(array, funct) {
    const partedYes = [];
    const partedNo = [];
    const partedTogether = [];
    _.each(array, function(element, pos, array) {
        if(funct(element, pos, array)) partedYes.push(element);
        else if(!funct(element, pos, array)) partedNo.push(element);
    });
    partedTogether.push(partedYes, partedNo);
    return partedTogether;
};
...
_.map = function(collection, funct) {
  ...
  _.each(collection, function(value, key, collection) {
      newValues.push(funct(value, key, collection));
  });
  return newValues;
};
...
_.every = function(collection, funct) {
    ...
    if(typeof(funct) === 'function') {
        _.each(collection, function(item, key, collection) {
            if(funct(item, key, collection) === false) {
                isItReal = false;
            }
        });
    }
    else(_.each(collection, function(item, key, collection) {
        if(!item) {
            isItReal = false;
        } 
    }))
...
_.some = function(collection, funct) {
    ...
    if(typeof(funct) === 'function') {
        _.each(collection, function(item, key, collection) {
            if(funct(item, key, collection) === true) {
                isItReal = true;
            }
        });
    }
